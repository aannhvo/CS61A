...
def powers(n, k):
    ...
    """
    def in_order(arg):
        """Yield all non-negative integers whose digits appear in order in arg.
        0 is yielded because 0 has no digits, so all its digits are in arg.
        """
        if arg == 0:
            yield 0
        else:
            for x in _____:
                _____
                _____
    yield from filter(curry2(_____)(_____), in_order(n))
